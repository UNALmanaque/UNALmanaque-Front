          ...
          <div class="row mb-3">
            ...
            <div class="form-group col-8">
              ...
              <select class="form-control" v-model="activity.category">
                <option
                  v-for="data in activity.categories"
                  :value="data.categoryId"
                  :key="data.categoryId"
                  >{{ data.categoryName }}</option
                >
                <!--toca cambiarlo cuando el get the categories funcione :c-->
              </select>
            </div>
          </div>
          <div class="row mb-3">
            <div class="form-group col-4">
              ...
              <label for="repetition"
                >Repetición:
                <input type="checkbox" value="" v-model="activity.repetition"
              /></label>
            </div>
  ...
  methods: {
    ...
    getCurrentUser: function() {
      ...
      if (user) {
        ...
        axios.get('/api/user/find/' + user.email).then((response) => {
